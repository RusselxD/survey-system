...
const CreateNewSurvey = (): React.JSX.Element => {
    const { toastError, toastSuccess, user } = useAuth();
    const navigate = useNavigate();

    const [locations, setLocations] = useState<IdNamePair[]>([]);
    const [serviceTypes, setServiceTypes] = useState<IdNamePair[]>([]);
    const [questionTypes, setQuestionTypes] = useState<IdNamePair[]>([]);
    ...
    const [askEmail, setAskEmail] = useState<boolean>(false);
