import React, { useState } from "react";
import { usersAPI } from "../../../../utils/api/models/users";
import ModalXButton from "../../../../components/reusable/ModalXButton";
import { useAuth } from "../../../../context/AuthContext";
import ConfirmDeleteContainer from "./ConfirmDeleteContainer";
import Roles from "./Roles";
import TextInput from "../../../../components/TextInput";

interface Role {
    id: number;
    name: string;
    description: string;
    userCount: number;
}

interface User {
    id: string;
    firstName: string;
    lastName: string;
    email: string;
    roleId: number;
    role?: string;
}

interface FormErrors {
    firstName?: string;
    lastName?: string;
    email?: string;
    roleId?: string;
}

interface DynamicDeleteUserButtonProps {
    confirmDeleteUserIsOpen: boolean;
    setConfirmDeleteUserIsOpen: React.Dispatch<React.SetStateAction<boolean>>;
    isUpdatingUser: boolean;
}

const DynamicDeleteUserButton = ({
    confirmDeleteUserIsOpen,
    setConfirmDeleteUserIsOpen,
    isUpdatingUser,
}: DynamicDeleteUserButtonProps) => {
    return (
        <button
            disabled={isUpdatingUser}
            onClick={() => setConfirmDeleteUserIsOpen(!confirmDeleteUserIsOpen)}
            className={` px-4 py-2 rounded-md transition-colors ${
                confirmDeleteUserIsOpen
                    ? "border dark:border-gray-300 border-gray-500 dark:hover:text-gray-800 hover:text-white dark:hover:bg-gray-300 hover:bg-gray-500"
                    : isUpdatingUser
                    ? "bg-red-600 text-white cursor-not-allowed opacity-50"
                    : "bg-red-600 hover:bg-red-700 text-white"
            }`}
        >
            {confirmDeleteUserIsOpen ? "Cancel" : "Delete User"}
        </button>
    );
};

interface ModifyUserModalProps {
    roles: Role[];
    userPassed: User;
    onClose: () => void;
    updateUserInList: (user: User) => void;
    deleteUserFromList: (userId: string) => void;
    updateRoleCountInList: (roleName: string, increment: boolean) => void;
}

const ModifyUserModal = ({
    roles,
    userPassed,
    onClose,
    updateUserInList,
    deleteUserFromList,
    updateRoleCountInList,
}: ModifyUserModalProps): React.JSX.Element => {
    const { toastError, toastSuccess } = useAuth();

    const oldRoleId: number | undefined = roles.find(
        (r) => r.id === userPassed.roleId
    )?.id;

    const [user, setUser] = useState<User>(userPassed);

    const [confirmDeleteUserIsOpen, setConfirmDeleteUserIsOpen] =
        useState<boolean>(false);

    const [errors, setErrors] = useState<FormErrors>({});
    const [isDeletingUser, setIsDeletingUser] = useState<boolean>(false);
    const [isUpdatingUser, setIsUpdatingUser] = useState<boolean>(false);

    const handleDeleteUser = async () => {
        setIsDeletingUser(true);

        try {
            await usersAPI.deleteUser(user.id);
            deleteUserFromList(user.id);
            const roleName =
                user.role ||
                roles.find((r) => r.id === user.roleId)?.name ||
                "";
            updateRoleCountInList(roleName, false);
            onClose();
            toastSuccess("User deleted successfully.");
        } catch (error: any) {
            console.log(error);
            toastError(
                error.response?.data?.message || "Something went wrong."
            );
        } finally {
            setIsDeletingUser(false);
        }
    };

    const handleUpdateUser = async () => {
        if (isUpdatingUser) return;

        // Validate form
        const newErrors: FormErrors = {};

        if (!user.firstName?.trim()) {
            newErrors.firstName = "First name is required";
        }

        if (!user.lastName?.trim()) {
            newErrors.lastName = "Last name is required";
        }

        if (!user.email?.trim()) {
            newErrors.email = "Email is required";
        } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(user.email)) {
            newErrors.email = "Please enter a valid email address";
        }

        if (!user.roleId) {
            newErrors.roleId = "Please select a role";
        }

        setErrors(newErrors);

        if (Object.keys(newErrors).length > 0) {
            return;
        }

        setIsUpdatingUser(true);

        // configuration based on UserAdminUpdateDto in backend
        const roleId = roles.find((role) => role.id === user.roleId)?.id;

        if (!roleId) {
            toastError("Invalid role selected.");
            setIsUpdatingUser(false);
            return;
        }

        const newUser: User = {
            id: user.id,
            firstName: user.firstName,
            lastName: user.lastName,
            email: user.email,
            roleId: roleId,
        };

        try {
            await usersAPI.updateUser(user.id, newUser);

            updateUserInList(newUser);

            if (oldRoleId !== undefined && oldRoleId !== newUser.roleId) {
                updateRoleCountInList(
                    roles.find((r) => r.id === oldRoleId)?.name || "",
                    false
                );
                updateRoleCountInList(
                    roles.find((r) => r.id === newUser.roleId)?.name || "",
                    true
                );
            }
            toastSuccess("User updated successfully.");
            onClose();
        } catch (error: any) {
            console.log(error);
            toastError(error.response?.data || "Something went wrong.");
        } finally {
            setIsUpdatingUser(false);
        }
    };

    return (
        <div className="modal modal-open" onClick={onClose}>
            <div
                className="modal-box pb-3 dark:bg-gray-700 lg:max-w-[45vw] max-h-[80vh] bg-white relative flex flex-col items-start px-4 sm:px-5"
                onClick={(e) => e.stopPropagation()}
            >
                <ModalXButton onClose={onClose} />

                <div className="custom-primary-txt -mt-2 border-b pb-2 border-base-content w-full">
                    <p className="truncate max-w-[90%]">
                        Edit User: {user.firstName}
                    </p>
                </div>

                <div className="w-full flex justify-between space-x-5 my-3">
                    <div className="w-full">
                        <TextInput
                            val={user.firstName}
                            setVal={(value: string) =>
                                setUser({ ...user, firstName: value })
                            }
                            label="First Name"
                            withLabel={true}
                        />
                        {errors.firstName && (
                            <p className="text-xs text-red-500 mt-1">
                                {errors.firstName}
                            </p>
                        )}
                    </div>
                    <div className="w-full">
                        <TextInput
                            val={user.lastName}
                            setVal={(value: string) =>
                                setUser({ ...user, lastName: value })
                            }
                            label="Last Name"
                            withLabel={true}
                        />
                        {errors.lastName && (
                            <p className="text-xs text-red-500 mt-1">
                                {errors.lastName}
                            </p>
                        )}
                    </div>
                </div>

                <div className="w-full">
                    <TextInput
                        val={user.email}
                        setVal={(value: string) =>
                            setUser({ ...user, email: value })
                        }
                        label="Email"
                        withLabel={true}
                    />
                    {errors.email && (
                        <p className="text-xs text-red-500 mt-1">
                            {errors.email}
                        </p>
                    )}
                </div>

                <Roles
                    roles={roles}
                    role={roles.find((r) => r.id === user.roleId)?.name || ""}
                    setRole={(value) => setUser({ ...user, roleId: value })}
                />
                {errors.roleId && (
                    <p className="text-xs text-red-500 mt-1">{errors.roleId}</p>
                )}

                <div className="flex items-center justify-between custom-primary-txt text-[0.800rem] mt-5 w-full">
                    <DynamicDeleteUserButton
                        confirmDeleteUserIsOpen={confirmDeleteUserIsOpen}
                        setConfirmDeleteUserIsOpen={setConfirmDeleteUserIsOpen}
                        isUpdatingUser={isUpdatingUser}
                    />
                    <div className="space-x-3 flex">
                        <button
                            onClick={() => onClose()}
                            disabled={confirmDeleteUserIsOpen}
                            className={` px-4 py-2 rounded-md transition-colors border dark:border-gray-300 border-gray-500  ${
                                confirmDeleteUserIsOpen
                                    ? "cursor-not-allowed opacity-50"
                                    : "dark:hover:text-gray-800 hover:text-white dark:hover:bg-gray-300 hover:bg-gray-500"
                            }`}
                        >
                            Cancel
                        </button>
                        <button
                            disabled={confirmDeleteUserIsOpen || isUpdatingUser}
                            onClick={() => handleUpdateUser()}
                            className={`bg-green-600 text-white px-4 py-2 rounded-md transition-colors  ${
                                confirmDeleteUserIsOpen
                                    ? "bg-gray-400  cursor-not-allowed opacity-50"
                                    : "hover:bg-green-700"
                            }`}
                        >
                            {isUpdatingUser ? (
                                <>
                                    <span className="loading loading-spinner loading-xs mr-2"></span>
                                    Saving...
                                </>
                            ) : (
                                <span>Save Changes</span>
                            )}
                        </button>
                    </div>
                </div>

                <ConfirmDeleteContainer
                    confirmDeleteUserIsOpen={confirmDeleteUserIsOpen}
                    handleDeleteUser={handleDeleteUser}
                    isDeletingUser={isDeletingUser}
                    user={user}
                />
            </div>
        </div>
    );
};

export default ModifyUserModal;
