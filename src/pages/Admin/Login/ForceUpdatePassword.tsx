...
interface DecodedToken {
    ...
    lastName: string;
    permissions?: string;
    forceUpdatePassword?: string;
    exp: number;
}

const ForceUpdatePassword = (): React.JSX.Element => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const res = await authAPI.forceUpdatePassword({
                newPassword,
            } as { newPassword: string });

            // The API returns a token in the response (API type may be incorrect)
