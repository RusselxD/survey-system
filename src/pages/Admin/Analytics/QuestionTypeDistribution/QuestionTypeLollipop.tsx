...
import { QuestionTypeDistribution } from "../../../../utils/api/pages/analytics";

interface QuestionTypeLollipopProps {
    dataset: QuestionTypeDistribution;
}
...
const QuestionTypeLollipop = ({
    dataset,
}: QuestionTypeLollipopProps): React.JSX.Element => {
    ...
    const sortedData = useMemo<SortedDataItem[]>(() => {
        const combined = questionTypes.map((type: string, index: number) => ({
            type,
            count: counts[index],
    ...
    return (
        <div className="w-full py-4">
            ...
            <div className="px-20 space-y-6">
                {sortedData.map((item: SortedDataItem, index: number) => {
