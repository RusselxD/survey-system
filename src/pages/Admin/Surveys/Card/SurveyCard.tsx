...
import { Survey } from "../../../../utils/api/models/survey";
    ...
    return (
        <span
            className={`absolute top-3 right-3 px-3 py-2 ${
                statusColors[statusLower] || "bg-gray-500"
            } text-white text-xs font-semibold rounded-lg uppercase shadow-md`}
        >
            {status}
        </span>
    );
...
interface ResponseAndQuestionsCountProps {
    responsesCount: number;
    questionsCount: number;
}

const ResponseAndQuestionsCount = ({
    responsesCount,
    questionsCount,
}: ResponseAndQuestionsCountProps): React.JSX.Element => {
    return (
...
const SurveyCard = ({ survey }: SurveyCardProps): React.JSX.Element => {
    const navigate = useNavigate();

    const [qrModalIsOpen, setQrModalIsOpen] = useState<boolean>(false);
