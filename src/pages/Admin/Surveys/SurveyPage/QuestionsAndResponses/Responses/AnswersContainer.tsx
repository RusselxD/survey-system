...
interface QuestionMetadata {
    ...
    columns?: Array<{ id: number; text: string }>;
    min?: number;
    max?: number;
}
...
const AnswerDisplay = ({ answer }: AnswerDisplayProps): React.JSX.Element => {
    const questionMetadata: QuestionMetadata = answer.questionMetadata
        ? JSON.parse(answer.questionMetadata)
        : {};
    const answerMetadata: any = answer.answerMetadata
        ...
        switch (answer.questionType) {
            ...
            case "dropdown": {
                const selectedOption = questionMetadata.options?.find(
                    (opt: any) => opt.id === answerMetadata?.selectedOptionId
                );
                return (
                    <p className="custom-sec-txt text-sm">
            ...
            case "checkboxes": {
                const selectedOptions = questionMetadata.options?.filter(
                    (opt: any) =>
                        answerMetadata?.selectedOptionIds?.includes(opt.id)
                );
                return (
            ...
            case "multiple_choice_grid": {
                ...
                return (
                    <div className="space-y-2">
                        {questionMetadata.rows?.map((row: any) => {
                    ...
                    <div className="space-y-2">
                        {questionMetadata.rows?.map((row: any) => {
                            ...
                            const selectedColumns =
                                questionMetadata.columns?.filter((col: any) =>
...
const AnswersContainer = ({
    answers,
    isFetching,
}: AnswersContainerProps): React.JSX.Element => {
    if (isFetching) {
        return (
