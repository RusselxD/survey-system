...
interface Responses {
    [key: string]: FormattedResponse | string;
}
...
const QuestionContainer = ({
    survey,
    responseId,
}: QuestionContainerProps): React.JSX.Element => {
    const navigate = useNavigate();
    const [responses, setResponses] = useState<Responses>({});
    const [errors, setErrors] = useState<Errors>({});
    // Store shuffled orders to prevent re-shuffling on re-renders
    const shuffledOrders = useRef<{ [key: string]: Option[] }>({});
    ...
    const valid = () => {
        ...
        if (
            survey.askName &&
            !(typeof responses.name === "string" && responses.name.trim())
        ) {
    ...
    const validateAndSubmit = async () => {
        ...
        try {
            const submitResponsePayload = {
                respondentName:
                    typeof responses.name === "string" ? responses.name : null,
                respondentEmail:
                    typeof responses.email === "string"
                        ? responses.email
                        : null,
                responseAnswers: Object.entries(responses)
                    .filter(([key]) => !isNaN(Number(key))) // Only include numeric keys
                    .map(([questionId, data]) => ({
                        questionId: parseInt(questionId),
                        answer:
                            (typeof data !== "string" && data.answer) || null,
                        metadata:
                            (typeof data !== "string" && data.metadata) || null,
                    })),
            };
    ...
    const parseMetadata = (metadataString: string | null): any => {
    ...
    const renderQuestion = (question: Question) => {
        ...
        switch (question.typeName) {
            ...
            case "multiple_choice":
                ...
                const mcMetadata = parseMetadata(
                    typeof mcResponse !== "string" ? mcResponse?.metadata : null
                );
            ...
            case "checkboxes":
                ...
                const cbMetadata = parseMetadata(
                    typeof cbResponse !== "string" ? cbResponse?.metadata : null
                );
            ...
            case "dropdown":
                ...
                const dropdownMetadata = parseMetadata(
                    typeof dropdownResponse !== "string"
            ...
            case "linear_scale":
                ...
                const linearScaleMetadata = parseMetadata(
                    typeof linearScaleResponse !== "string"
            ...
            case "multiple_choice_grid":
                ...
                const gridResponses =
                    parseMetadata(
                        typeof mcGridResponse !== "string"
                            ? mcGridResponse?.metadata
                            : null
                    ) || {};
            ...
            case "checkbox_grid":
                ...
                const cbGridResponses =
                    parseMetadata(
                        typeof cbGridResponse !== "string"
                            ? cbGridResponse?.metadata
                            : null
                    ) || {};
