...
const QuestionEditor = ({
    ...
    const updateOption = (
        optionId: number,
        newText: string,
        arrayKey: string = "options"
    ) => {
        const currentArray = metadata[arrayKey] || [];
        const updated = currentArray.map((opt: any) =>
    ...
    const duplicateOption = (
        ...
        const currentArray = metadata[arrayKey] || [];
        const index = currentArray.findIndex((opt: any) => opt.id === optionId);
    ...
    const removeOption = (optionId: number, arrayKey: string = "options") => {
        const currentArray = metadata[arrayKey] || [];
        const updated = currentArray.filter((opt: any) => opt.id !== optionId);
        updateMetadata({ ...metadata, [arrayKey]: updated });
    };
