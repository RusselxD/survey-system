import { BarChart3, FileText, MessageSquare, LucideIcon } from "lucide-react";
import { DashboardMetrics } from "../../../utils/api/pages/dashboard";

interface SurveyCompositionItem {
    title: string;
    desc: string;
    data: string | number;
    icon: LucideIcon;
...
const surveyCompositionData: SurveyCompositionItem[] = [
    {
        title: "Total Surveys",
        desc: "all time",
...
interface SurveyCompositionProps {
    data: DashboardMetrics | null;
}

const SurveyComposition = ({ data }: SurveyCompositionProps): React.JSX.Element => {
    const { totalSurveys, avgQuestions, avgResponses } = data || {};

    // Create a copy to avoid mutating the original array
    const compositionData: SurveyCompositionItem[] = [
        {
            ...surveyCompositionData[0],
            data: totalSurveys ? totalSurveys.toLocaleString() : "0",
        },
        {
            ...surveyCompositionData[1],
            data: avgQuestions || 0,
        },
        {
            ...surveyCompositionData[2],
            data: avgResponses ? avgResponses.toLocaleString() : "0",
        },
    ];
    return (
